...
### Fixed
- Ship portable Linux binary via musl (x86_64-unknown-linux-musl) for broad GLIBC compat
- Align release archives: Unix .tar.gz, Windows .zip
- Fix npm installer to download and extract archives correctly
- Use stable Intel macOS runner (macos-15-intel) for darwin x64 builds
- Remove `mentioned` filter that blocked ALL direct message delivery

### Changed
- npm wrapper version aligned to match release version
